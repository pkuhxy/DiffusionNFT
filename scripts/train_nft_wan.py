...
def eval_fn(
    ...
    for test_batch in tqdm(
        ...
        rewards_future = executor.submit(reward_fn, videos, prompts, prompt_metadata, only_strict=False)
        time.sleep(0)
    ...
    if is_main_process(rank):
        ...
        videos_to_log = videos


        with tempfile.TemporaryDirectory() as tmpdir:
            num_samples_to_log = min(15, len(videos_to_log))
            sample_indices = random.sample(range(len(videos_to_log)), num_samples_to_log)

            for idx, i in enumerate(sample_indices):
                video = videos_to_log[i]
...
def main(_):
    ...
    if config.use_lora:
        ...
        if config.train.lora_path:
            pipeline.transformer = PeftModel.from_pretrained(transformer, config.train.lora_path)
            # 使用PeftModel.from_pretrained load后所有参数的requires_grad都是False，需要set_adapter来使得adapter参数梯度为True
            pipeline.transformer.set_adapter("default")
        else:
            pipeline.transformer = get_peft_model(transformer, transformer_lora_config)
    ...
    # --- Prompt Embeddings ---
    neg_prompt_embed = compute_text_embeddings([""], text_encoders, tokenizers, max_sequence_length=512, device=device)
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for i in tqdm(
            ...
            rewards_future = executor.submit(reward_fn, videos, prompts, prompt_metadata, only_strict=True)
            time.sleep(0)
        ...
        collated_samples["rewards"]["avg"] = (
            collated_samples["rewards"]["avg"].unsqueeze(1) - config.sample.kl_reward*collated_samples["kl"]
        )
        ...
        if is_main_process(rank):  # logging
            ...
            {
                "epoch": epoch,
                **{f"reward_{key}": value.mean() for key, value in gathered_rewards_dict.items() if '_strict_accuracy' not in key and '_accuracy' not in key},
                "kl": collated_samples["kl"].mean().cpu().numpy(),
                "kl_abs": collated_samples["kl"].abs().mean().cpu().numpy()
            },
                step=global_step,
            )
        ...
        if config.per_prompt_stat_tracking:
            ...
            if is_main_process(rank):
                ...
                zero_std_ratio = calculate_zero_std_ratio(prompts, gathered_rewards_dict)
                wandb.log(
                    {
